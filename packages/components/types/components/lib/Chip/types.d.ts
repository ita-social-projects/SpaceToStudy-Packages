...
type ChipColor = 'blue' | 'blue-gray' | 'green' | 'neutral' | 'purple' | 'red' | 'turquoise' | 'yellow';
type ChipWithLabel = {
    label: string;
};
type ChipWithIcons = {
...
type FilterChipProps = CommonChipProps<'filter'> & ChipWithLabel & ChipWithIcons & {
...
type InputChipProps = CommonChipProps<'input'> & ChipWithLabel & ChipWithIcons & {
    onRemoveButtonClick?: () => void;
    variant?: 'filled' | 'outlined' | 'filled-outlined';
};
type CategoryChipProps = CommonChipProps<'category'> & ChipWithLabel & {
    color?: ChipColor;
    detail: string;
};
type StateChipProps = CommonChipProps<'state'> & ChipWithLabel & Pick<ChipWithIcons, 'startIcon'> & {
    color?: ChipColor;
};
